...
function SCROLLBAR:Think()
	...
	if ((mx > -8 and mx < self:GetWide() + 3 and my > self.scrollButton.y and my < self.scrollButton.y + self.height) or self.scrollButton.scrolling) then
		self.addWidth = math.Clamp(self.addWidth + (FrameTime() * 52), 0, 3)
	else
		self.addWidth = math.Clamp(self.addWidth - (FrameTime() * 52), 0, 3)
	end
...
function SCROLLBAR:PaintScrollPanel(self, w, h) end
function SCROLLBAR:PaintVScrollBar(self, w, h) end
...
vgui.Register('WDScrollPanel', SCROLLABLE, 'Panel')
...
function PANEL:Init()
	...
	self.lblTitle:SetFont("Default")
	timer.Simple(0.2, function()
		if !self.NoPopUp then
...
vgui.Register("WDFrame", PANEL, "DFrame")
...
function PANEL:Init()
	...
	self:MakePopup()
	self.Question = vgui.Create("DLabel", self)
	self.Question:SetFont("Trebuchet18")
...
function PANEL:SetQuestion(question)
	...
	self.Question:SetPos(5, 5)
end

function PANEL:PerformLayout()
	self:SetSize(370, 80)
	self:Center()
	self.Yes:SetSize(self:GetWide() / 2, 25)
	self.Yes:SetPos(0, self:GetTall() - self.Yes:GetTall())
	self.No:SetSize(self:GetWide() / 2 + 1, 25)
	self.No:SetPos(self.Yes:GetWide() - 1, self:GetTall() - self.No:GetTall())
end
vgui.Register("WDConfirm", PANEL, "Panel")